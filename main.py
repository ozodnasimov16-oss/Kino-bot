...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if message.text and message.text.startswith('/start code_'):
        try:
            ...
            await send_movie_to_user(message.from_user.id, movie_code)
            return
...
async def send_movie_to_user(user_id: int, movie_code: int):
...
async def on_startup():
    ...
    try:
        ...
        logger.warning(f"Admin xabar yuborishda xatolik: {e}")
